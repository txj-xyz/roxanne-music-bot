import { Channel, EmbedBuilder, Guild, GuildMember } from 'discord.js';
import { Shoukaku, Connectors, Player, Node, Track, WebSocketClosedEvent } from 'shoukaku';
import Bot from '../Bot';
import { BotModule } from '../handlers/ModuleHandler';

export const enabled = true;
export const name = 'Music';
export default interface Music extends Shoukaku {}
export interface MusicDispatcher {
    ...
    stopped: boolean;
    nowplaying: null;
    playingEmbed(message: string | { name: string; track: Track }): EmbedBuilder;
...
export class MusicDispatcher {
    constructor(client: Bot, guild: Guild, channel: Channel, player: Player) {
        ...
        this.player
            .on('start', async () => client.logger.log({ handler: this.constructor.name, message: `Track Started` }, false))
            .on('end', async () => client.logger.log({ handler: this.constructor.name, message: `Track Ended` }, false))
            .on('stuck', () => client.logger.log({ handler: this.constructor.name, message: `Track Stuck` }, false))
            .on('closed', async (payload: WebSocketClosedEvent) => client.logger.log({ handler: this.constructor.name, message: `Track closed with payload: ${payload.code}` }, false));
    }
}
