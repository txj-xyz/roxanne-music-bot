...
class PlayNext extends RoxanneInteraction {
    ...
    async run({ interaction, dispatcher }) {
        ...
        // if (query.includes('apple.com')) return await interaction.editReply('Sorry human, Apple Music is not available at the moment for this command. <:sad:585678099069403148>');
        ...
        //     } else {
        //         await interaction
        //             .editReply(`Queueing \`${String(playlist.tracks.length)}\` tracks from \`${playlist.playlistInfo?.name || playlist.title}\` to play next in the queue!`)
        //             .catch(() => null);
        //     }
        ...
        //     if (!result) return interaction.editReply("I didn't find any song on the query you provided!");
        ...
        // await interaction.editReply(`Added the track \`${track.info.title}\` to play next in the queue!`).catch(() => null);
        // dispatcher2?.play();
    }
}
