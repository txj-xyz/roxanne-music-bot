    ...
    async run({ interaction }) {
        ...
        if (
            ...
            if (playlist.loadType === 'TRACK_LOADED') {
                await interaction
                    .editReply(
                        `\`${firstTrack.info.author} - ${firstTrack.info.title}\` added to the Queue!`
                    )
                    .catch(() => null);
            } else {
                await interaction
        ...
        if (Play.checkURL(query)) {
            ...
            if (!track?.info) {
                return await interaction.editReply(
                    'There was an error finding the song information, please try again.'
                );
            }
            ...
            await interaction
                .editReply(
                    playlist
                        ? `Added the playlist \`${playlistName}\` in queue!`
                        : `Added the track \`${track?.info.title}\` in queue!`
                )
                .catch(() => null);
            dispatcher?.play();
            return;
