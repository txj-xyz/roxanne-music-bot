...
export default class EventHandler {
    constructor(client: Bot) {
        ...
        client.on('shardReady', (id) => this.client.logger.log({ message: `Shard ${id} | Shard Ready`, handler: this.constructor.name }, true));
    }
        ...
        for (const event of events) {
            if (event.endsWith('.ts')) {
                import(`${this.client.location}/src/events/${event}`).then((event) => {
                    ...
                    this.client.logger.log({ message: `Event '${botEvent.name}' loaded.`, handler: this.constructor.name }, false);
                    if (botEvent.enabled) {
